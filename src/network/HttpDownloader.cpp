...
// CDN sends more and logs HTTP_HEADER "Buffer length is small". Do NOT shrink
// this to save heap: 2048 was tried, and the font manifest -- a GitHub release
// asset, so a 302 to a CDN whose signed Location header is several hundred
// bytes on its own, parsed again after the redirect -- crashed the device
// outright with "assert failed: http_utils_append_string http_utils.c:72".
// CrossPoint 1.6 does ship 2048, but it does not fetch this manifest through
// this path, so its choice is not evidence that 2048 is safe here. TX likewise
// has to hold the request line for that signed CDN URL after a redirect.
// The body streams separately in READ_CHUNK pieces.
